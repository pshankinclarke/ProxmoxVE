...
Debian does not enable AOF by default. Once enabled, AOF logs write every operation by the server. On restart, Valkey will replay the log to rebuild your dataset.
...
4.) RDB + AOF hybrid:
If you want something similar to what PostgreSQL offers, turn on RDB snapshots and AOF.
